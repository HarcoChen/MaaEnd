...
func (a *SelectItemAction) Run(ctx *maa.Context, arg *maa.CustomActionArg) bool {
	...
	if shouldRouteSkip(result.AbortReason) {
		return routeSkipWithAbortReason(ctx, arg.CurrentTaskName, result.AbortReason, nil, i18n.T("buygoodselastic.recognition_early_end"))
	}
	...
	if !selection.Selected {
		...
		maafocus.Print(ctx, i18n.T("buygoodselastic.no_qualifying_product", selection.Reason))
	...
	if quantityDecision.Mode == quantityModeSkip {
		...
		maafocus.Print(ctx, i18n.T("buygoodselastic.hit_but_skip", quantityDecision.Reason))
		if err := overrideSkipBranch(ctx); err != nil {
			log.Error().
				Err(err).
				Str("component", "buygoodselastic").
	...
	maafocus.Print(ctx, i18n.T("buygoodselastic.product_selected", selectionMode, selection.ProductName, selection.CurrentPrice))
...
func SelectBestProduct(data RecognitionData, cfg SelectionConfig, bypassThresholdFilter bool) (SelectionResult, error) {
	if len(data.Goods) == 0 {
		return SelectionResult{Selected: false, Reason: i18n.T("buygoodselastic.no_goods_recognized")}, nil
	}
	...
	if len(candidates) == 0 {
		return SelectionResult{Selected: false, Reason: i18n.T("buygoodselastic.no_qualifying_goods")}, nil
	}
...
func routeSkipWithAbortReason(ctx *maa.Context, currentTaskName string, reason AbortReason, err error, focusPrefix string) bool {
	...
	if reason.isWarn() {
		maafocus.Print(ctx, i18n.RenderHTML("buygoodselastic.warning_skip", map[string]any{
			"Prefix": focusPrefix,
			"Reason": reasonText,
		}))
	} else {
		maafocus.Print(ctx, i18n.T("buygoodselastic.abort_info", focusPrefix, reasonText))
	}
	if err := overrideSkipBranch(ctx); err != nil {
		log.Error().
			Err(err).
...
func stopTaskWithFocus(ctx *maa.Context, reason AbortReason, err error) bool {
	...
	maafocus.Print(ctx, i18n.RenderHTML("buygoodselastic.fatal_error", map[string]any{
		"Reason": reasonText,
	}))
...
func formatSelectionMode(selection SelectionResult, data RecognitionData) string {
	if selection.CurrentPrice < selection.Threshold {
		return i18n.T("buygoodselastic.mode_low_price")
	}
	if data.Quota.Overflow > 0 {
		return i18n.T("buygoodselastic.mode_overflow")
	}
	return i18n.T("buygoodselastic.mode_low_price")
}
