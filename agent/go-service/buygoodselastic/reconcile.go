...
func (a *ReconcileDecisionAction) Run(ctx *maa.Context, arg *maa.CustomActionArg) bool {
	...
	if !priceChanged {
		if err := enableDecisionReadyOverride(ctx); err != nil {
			log.Error().
				Err(err).
				Str("component", "buygoodselastic").
				Str("node", "BuyGoodsElasticRelayNodeDecisionReady").
	...
	if priceChanged {
		maafocus.Print(ctx, i18n.T("buygoodselastic.reconcile_price_corrected", oldPrice, price))
	}
	...
	if isEquivalent {
		...
		maafocus.Print(ctx, i18n.T("buygoodselastic.reconcile_decision_unchanged"))
	...
	if !newSelection.Selected {
		...
		maafocus.Print(ctx, i18n.T("buygoodselastic.no_qualifying_product", newSelection.Reason))
	...
	maafocus.Print(ctx, i18n.T("buygoodselastic.product_selected", formatSelectionMode(newSelection, updatedData), newSelection.ProductName, newSelection.CurrentPrice))
